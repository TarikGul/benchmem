...
describe('mem', () => {
	...
	describe('gCProfilerWrapper', () => {
		...
		it('should execute function specified number of iterations for sync', async () => {
			const testFn = jest.fn().mockReturnValue('test');
		...
		it('should execute function specified number of iterations for async', async () => {
			const testFn = jest.fn().mockResolvedValue('test');
			
			const resultPromise = gCProfilerWrapper(testFn, { iterations: 3, async: true });
			jest.advanceTimersByTime(1);
			await resultPromise;
		...
		it('should start and stop GC profiler', async () => {
			const testFn = jest.fn().mockReturnValue('test');
			const mockProfiler = {
				start: jest.fn(),
				stop: jest.fn().mockReturnValue(mockGCProfilerResult),
		...
		it('should handle zero iterations', async () => {
			...
			const resultPromise = gCProfilerWrapper(testFn, { iterations: 0 });
			jest.advanceTimersByTime(1);
			await resultPromise;
